...
test.describe("Contact Form Tests @business-logic", () => {
  test.beforeEach(async ({ page }) => {
    ...
    await page.route('**/api/v1/agents', async route => {
      const response = await fetch('https://wre-server-production.up.railway.app/api/v1/agents');
  ...
  test("Submit Contact Form", async ({ page }) => {
    ...
    await page.waitForFunction(() => {
      const select = document.querySelector('[data-testid="contact-form-agent"]');
    ...
    await page.fill('[data-testid="contact-form-email"]', "timwillie73@gmail.com");
    ...
    await expect(page.locator('text=Introduction email sent to agent!')).toBeVisible({ timeout: 30000 });
  });
});